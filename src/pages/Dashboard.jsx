...
function DermatologistCard({ doc }) {
  return (
    <div className="derm-card">
      <div className="derm-card-top">
        <span className="derm-name">{doc.name}</span>
        {doc.phone && (
          <a href={`tel:${doc.phone}`} className="derm-phone">{doc.phone}</a>
        )}
      </div>
      {doc.specialty && <div className="derm-specialty">{doc.specialty}</div>}
      {doc.hospital && <div className="derm-hospital">{doc.hospital}</div>}
      {doc.availability && <div className="derm-availability">{doc.availability}</div>}
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const handleDownloadPDF = () => {
    ...
    // Recommended dermatologists — own page so it never collides with the layout above
    if (result.recommended_dermatologists && result.recommended_dermatologists.length > 0) {
      doc.addPage();
      doc.setFillColor(10, 12, 15); doc.rect(0, 0, 210, 297, 'F');
      doc.setFont('helvetica','bold'); doc.setFontSize(16); doc.setTextColor(232,237,245);
      doc.text('Recommended Dermatologists', 20, 28);

      let dy = 44;
      result.recommended_dermatologists.forEach(rec => {
        if (dy > 260) {
          doc.addPage();
          doc.setFillColor(10, 12, 15); doc.rect(0, 0, 210, 297, 'F');
          dy = 28;
        }
        doc.setFont('helvetica','bold'); doc.setFontSize(13); doc.setTextColor(232,237,245);
        doc.text(rec.name || 'Dermatologist', 20, dy);
        doc.setFont('helvetica','normal'); doc.setFontSize(10); doc.setTextColor(138,155,181);
        doc.text(rec.specialty || '', 20, dy + 8);
        doc.text(rec.hospital || '', 20, dy + 16);
        doc.text(rec.availability || '', 20, dy + 24);
        doc.text(rec.phone || '', 20, dy + 32);
        dy += 46;
      });
    }
        ...
        {/* Result panel */}
          ...
          {result && (
            <div className="result-content">
              ...
              {result.recommended_dermatologists && result.recommended_dermatologists.length > 0 && (
                <div className="derm-section">
                  <div className="card-label" style={{ marginBottom: 10 }}>Recommended Dermatologists</div>
                  <div className="derm-list">
                    {result.recommended_dermatologists.map(rec => (
                      <DermatologistCard key={rec.id} doc={rec} />
                    ))}
                  </div>
                </div>
              )}

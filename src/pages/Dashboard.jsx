...
// Longer/more-specific names first — prevents "Melanocytic Nevi" matching "mel" in Melanoma
const LESION_MAP = [
...
function getLesion(predictedClass) {
  ...
  // Partial match — longer names listed first prevents mel matching melanocytic nevi
...
export default function Dashboard() {
  ...
  const onDrop = useCallback(accepted => {
    ...
    if (!['png','jpg','jpeg'].includes(ext)) { setError('Only .png and .jpg files are accepted.'); return; }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.log('Predict response:', data); // helps debug field names
      setResult(data);
    } catch (err) {
  ...
  const handleReset = () => { setPreview(null); setFile(null); setResult(null); setError(''); };
  ...
  const handleDownloadPDF = () => {
    ...
    if (result.all_probabilities) {
      doc.setFontSize(13); doc.setFont('helvetica','bold'); doc.setTextColor(138,155,181);
      doc.text('CLASS PROBABILITIES', 20, 138);
      let y = 150;
      Object.entries(result.all_probabilities).forEach(([cls, prob]) => {
        doc.setFont('helvetica','normal'); doc.setFontSize(11); doc.setTextColor(232,237,245);
        doc.text(`${cls}: ${Math.round(prob * 100)}%`, 20, y);
        y += 10;
      });
    }
    // Disclaimer
    doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(138,155,181);
    doc.text('DISCLAIMER', 20, 200);
    ...
    doc.text(doc.splitTextToSize(disc, 170), 20, 212);
    doc.save(`dermavision-report-${Date.now()}.pdf`);
  };

  // Parse result using flexible name matching
        ...
        <div className="card">
          ...
          {!preview ? (
            ...
            <div className="preview-wrap">
              ...
              <button className="reset-btn" onClick={handleReset}><RotateCcw size={14} /> Reset</button>
            </div>
          )}
          {error && (
            ...
            <div className="result-content">
              ...
              {result.all_probabilities && (
                <div className="scores-grid">
                  {Object.entries(result.all_probabilities).map(([k, v]) => (
